...
public class Player2Controls : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (col == null)
        {
            ...
            Debug.LogWarning("Player2Controls: No Collider found on player � added a default CapsuleCollider. Adjust in Inspector if needed.");
        }
        ...
        Debug.Log($"Player2 initialized with {currentHealth}/{maxHealth} health and {currentSpecialAttack}/{maxSpecialAttack} special attack");
    }
    ...
    void Update()
    {
        ...
        // Horizontal
        if (Input.GetKey(KeyCode.J)) h = -1f;     // Left
        if (Input.GetKey(KeyCode.L)) h =  1f;     // Right

        // Vertical
        if (Input.GetKey(KeyCode.I)) v =  1f;     // Up
        if (Input.GetKey(KeyCode.K)) v = -1f;     // Down
    ...
    private System.Collections.IEnumerator PerformSpecialAttack()
    {
        ...
        {
            BasicEnemyControls enemyAI = enemy.GetComponentInParent<BasicEnemyControls>();
